...
use iced::{Color as IcedColor, Point, Size};

/// Chart renderer — draws all chart elements using the IcedBackend.
///
/// All functions take `&mut IcedBackend<'a>` directly. There is no
/// trait-dispatch layer; if a future backend is needed, add a new
/// `IcedBackend` adapter in `renderers/`.
use crate::chart_state::ChartState;
use crate::color::{Color, Palette};
use crate::overlays::{LineStyle, MarkerPosition, MarkerShape};
use crate::renderers::iced::{snap_x, snap_y, IcedBackend};
...
fn draw_grid<'a>(b: &mut IcedBackend<'a>, state: &ChartState, time_ticks: &[TickMark]) {
...
fn draw_ohlc_bars<'a>(b: &mut IcedBackend<'a>, pane_index: usize, state: &ChartState) {
...
/// Called inside `with_clip` — lines are clipped to the pane's bounds.
fn draw_price_lines<'a>(b: &mut IcedBackend<'a>, state: &ChartState, pane_idx: usize) {
...
fn draw_price_line_labels<'a>(b: &mut IcedBackend<'a>, state: &ChartState) {
...
fn draw_series_markers<'a>(b: &mut IcedBackend<'a>, state: &ChartState) {
    let plot = &state.layout.plot_area;
...
fn draw_watermark<'a>(b: &mut IcedBackend<'a>, state: &ChartState) {
...
fn draw_last_value_marker<'a>(b: &mut IcedBackend<'a>, state: &ChartState, pane_idx: usize) {
    let pane = &state.panes[pane_idx];
    let plot = &pane.layout_rect;
...
fn draw_last_value_label<'a>(b: &mut IcedBackend<'a>, state: &ChartState) {
    let pane = &state.panes[0];
    let plot = &pane.layout_rect;
...
fn draw_candlestick_bars<'a>(b: &mut IcedBackend<'a>, pane_index: usize, state: &ChartState) {
...
fn draw_candlestick_bars_data<'a>(
    b: &mut IcedBackend<'a>,
    pane_index: usize,
    state: &ChartState,
...
fn draw_line_series_from_ohlc<'a>(b: &mut IcedBackend<'a>, pane_index: usize, state: &ChartState) {
    let pane = &state.panes[pane_index];
    let plot_area = &pane.layout_rect;
